...
func (s *service) GetByID(id primitive.ObjectID) (*ct.PostResponse, error) {
...
func (s *service) Create(req *ct.CreatePostRequest, userID primitive.ObjectID) (*ct.PostResponse, error) {
	...
	if len(req.Tags) > 0 {
		tagIDs = []primitive.ObjectID(req.Tags)
		tags, err := s.tagRepo.Select(ctx, tagIDs)
	...
	fullPost, err := s.postRepo.ReadByCondition(
		map[string]interface{}{"_id": res.ID},
		"User", "Tags",
	)
	if err != nil {
...
func (s *service) Update(ctxUserID primitive.ObjectID, updatePost *ct.UpdatePostRequest) (*ct.PostResponse, error) {
	ctx := context.Background()
	...
	tagIDs := []primitive.ObjectID(updatePost.Tags)
	tags, err := s.tagRepo.Select(ctx, tagIDs)
	if err != nil {
		return nil, err
	}
...
func (s *service) Delete(postID, ctxUserID primitive.ObjectID) error {
	post, err := s.postRepo.ReadByCondition(map[string]any{
		"_id": postID,
	})
	if err != nil {
	...
	// Check ctxUser permission to delete
	if ctxUserID != post.UserID {
